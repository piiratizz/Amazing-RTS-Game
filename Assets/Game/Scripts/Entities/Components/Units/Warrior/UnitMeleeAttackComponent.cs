...
public class UnitMeleeAttackComponent : EntityComponent, IAttackable
{
    ...
    [SerializeField] private float findClosestEnemyDelay = 0.5f;
    ...
    private void AttackUnit(Unit unit, int unitCircleOffset = 0, int unitsInFormation = 1)
    {
        if (unit == null) return;
        ...
        _targetUnitDamageResistanceComponent = unit.GetEntityComponent<UnitDamageResistanceComponent>();
        var health = unit.GetEntityComponent<HealthComponent>();

...
#pragma warning disable 0649
    ...
    internal class ConsoleGui
    {
        ...
        private Rect inputFieldRect => new Rect(5, rect_console.y + rect_console.height + 5, 400,
            skin.textField.CalcHeight(new GUIContent("Input"), 50));
        ...
        internal ConsoleGui(ConsoleBackend backend, Options options)
        {
            ...
            m_options.InputActionAsset.FindAction("ToggleConsole").performed +=
                _ => InputToggleConsole = !InputToggleConsole;
        }
        ...
        private void ControlInputField()
        {
            ...
            // === 1. Проверяем нажатие клавиши для открытия консоли ===
            if (e.type == EventType.KeyDown)
            {
                var toggleAction = m_options.InputActionAsset.FindAction("ToggleConsole");
                if (toggleAction != null)
                {
                    // Берем первый биндинг (например "Keyboard/F1")
                    var binding = toggleAction.bindings.FirstOrDefault();
                    if (!string.IsNullOrEmpty(binding.path) && binding.path.StartsWith("<Keyboard>/"))
                    {
                        // Извлекаем имя клавиши (например, "f1")
                        string keyName = binding.path.Replace("<Keyboard>/", "");
                        if (Enum.TryParse(keyName, true, out KeyCode key))
                        {
                            if (!consoleWasOpened && e.keyCode == key)
                            {
            ...
            // === 2. Обработка нажатий внутри поля ввода ===
            if (e.type == EventType.KeyDown)
            {
                if (GUI.GetNameOfFocusedControl() == INPUT_FIELD_NAME)
                {
                    if (e.keyCode == KeyCode.Return)
                    {
                        ...
                        var input = currentSuggestionIndex == -1 ? console_input : currentSuggestion;
        ...
        private void DrawAutoCompleteSuggestions(string str)
        {
            ...
            foreach (var item in results)
            {
                ...
                if (!string.IsNullOrEmpty(description))
                {
                    ...
                    if (currentSuggestionIndex == num)
                    {
                        GUI.Label(new Rect(pos.x + pos.width + 5F, pos.y, descriptionSize.x, descriptionSize.y),
        ...
        {
            return new Rect(rect.x + padding, rect.y + padding, rect.width - padding - padding,
